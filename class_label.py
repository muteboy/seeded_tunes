...
class label(object):
    def __init__(self, scene, number):
        ...
        self.yearFirst = randint(yearStart, yearStart+((self.scene.yearNow-yearStart)/2))
        self.yearLast = randint(self.yearFirst, self.scene.yearNow)
        self.years = []
        self.numArtists = randint(1, 6) if self.scene.numArtists==0 else self.scene.numArtists
        self.seed = int(str(scene.seed) + format(self.number, "02d"))
        seed(self.seed)
        nameSuffixes=["Recordings", "Records", "", "Group", "Committee", "Music"]
        self.name = f"{self.scene.wordList.combineRandomLinesFromFile(numWords=1)} {choice(nameSuffixes)}"
        ...
        self.artists = []
        self.albumCatalog = []
        for num in tqdm(range(1, self.numArtists + 1), desc="03 Artists".ljust(18), position=3):
    ...
    def graph(self):
        ...
        for p in self.people:
            nl += '<tr><td align="left" border="0"><font point-size="9">'+ p.fullname +'</font></td></tr>'
        nl += '</table>>'
        self.scene.gvGraph.node(name=self.name,label=nl,shape="plaintext",margin="0")
        
    def html(self):
        _div = div(id=self.name)
        _div += h2(f"Label: {self.name}")
        _div += p(self.personnel())
        _div += p(self.comment())
        return _div
